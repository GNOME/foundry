 ...
 *
 * Copyright 2015-2025 Christian Hergert <christian@sourceandstack.com>
 *
...
G_DEFINE_FINAL_TYPE (FoundryMenuManager, foundry_menu_manager, G_TYPE_OBJECT)

#define PANEL_MENU_ATTRIBUTE_BEFORE   "before"
#define PANEL_MENU_ATTRIBUTE_AFTER    "after"
#define PANEL_MENU_ATTRIBUTE_MERGE_ID "libfoundry-merge-id"

/**
 * FoundryMenuManager:
 *
 * The goal of #FoundryMenuManager is to simplify the process of merging multiple
 ...
 * submenu links. This allows the #FoundryMenuManager to be in full control of
 * the generated menus.
 *
 * foundry_menu_manager_get_menu_by_id() will always return a #GMenu, however
...
static void
foundry_menu_manager_dispose (GObject *object)
{
  FoundryMenuManager *self = (FoundryMenuManager *)object;

  g_clear_pointer (&self->models, g_hash_table_unref);

  G_OBJECT_CLASS (foundry_menu_manager_parent_class)->dispose (object);
}

static void
foundry_menu_manager_class_init (FoundryMenuManagerClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = foundry_menu_manager_dispose;
}

static void
foundry_menu_manager_init (FoundryMenuManager *self)
{
...
static gboolean
foundry_menu_manager_menu_contains (FoundryMenuManager *self,
                                    GMenu              *menu,
                                    GMenuItem          *item)
{
  const char *id;
  ...
  g_assert (FOUNDRY_IS_MENU_MANAGER (self));
...
static void
foundry_menu_manager_resolve_constraints (GMenu *menu)
{
      ...
      for (gint j = n_items - 1; j > i; j--)
        {
          ...
          if (foundry_str_equal0 (i_after, j_id) || foundry_str_equal0 (i_after, j_label))
            {
      ...
      for (gint j = 0; j < i; j++)
        {
          ...
          if (foundry_str_equal0 (i_before, j_id) || foundry_str_equal0 (i_before, j_label))
            {
              /*
...
static void
foundry_menu_manager_add_to_menu (FoundryMenuManager *self,
                                  GMenu              *menu,
                                  GMenuItem          *item)
{
  g_assert (FOUNDRY_IS_MENU_MANAGER (self));
  ...
  foundry_menu_manager_resolve_constraints (menu);
  foundry_menu_manager_resolve_constraints (menu);
}

static void
foundry_menu_manager_merge_model (FoundryMenuManager *self,
                                  GMenu              *menu,
                                  GMenuModel         *model,
                                  guint               merge_id)
{
  ...
  for (guint i = 0; i < n_items; i++)
    {
       ...
       */
      if (foundry_menu_manager_menu_contains (self, menu, item))
        {
          g_object_unref (item);
          continue;
        }

      foundry_menu_manager_add_to_menu (self, menu, item);
...
static void
foundry_menu_manager_merge_builder (FoundryMenuManager *self,
                                    GtkBuilder         *builder,
                                    guint               merge_id)
{
  const GSList *iter;
  ...
  for (iter = list; iter != NULL; iter = iter->next)
    {
      ...
      foundry_menu_manager_merge_model (self, menu, G_MENU_MODEL (object), merge_id);
    }
...
FoundryMenuManager *
foundry_menu_manager_new (void)
{
  return g_object_new (FOUNDRY_TYPE_MENU_MANAGER, NULL);
}

/**
 * foundry_menu_manager_get_default:
 *
 * Returns: (transfer none):
 */
FoundryMenuManager *
foundry_menu_manager_get_default (void)
{
  static FoundryMenuManager *instance;

  if (instance == NULL)
    {
      instance = foundry_menu_manager_new ();
...
guint
foundry_menu_manager_add_filename (FoundryMenuManager  *self,
                                   ...
                                   GError             **error)
{
  ...
  g_return_val_if_fail (FOUNDRY_IS_MENU_MANAGER (self), 0);
  ...
  foundry_menu_manager_merge_builder (self, builder, merge_id);
  g_object_unref (builder);
...
/**
 * foundry_menu_manager_merge:
 * @self: a #FoundryMenuManager
 ...
 */
guint
foundry_menu_manager_merge (FoundryMenuManager *self,
                            const gchar        *menu_id,
                            GMenuModel         *menu_model)
{
  ...
  foundry_menu_manager_merge_model (self, menu, menu_model, merge_id);

  return merge_id;
}

/**
 * foundry_menu_manager_remove:
 * @self: a #FoundryMenuManager
 ...
 */
void
foundry_menu_manager_remove (FoundryMenuManager *self,
                             guint               merge_id)
{
  GHashTableIter iter;
  GMenu *menu;

  g_return_if_fail (FOUNDRY_IS_MENU_MANAGER (self));
...
/**
 * foundry_menu_manager_get_menu_by_id:
 *
 ...
 */
GMenu *
foundry_menu_manager_get_menu_by_id (FoundryMenuManager *self,
                                     const gchar        *menu_id)
{
  GMenu *menu;

  g_return_val_if_fail (FOUNDRY_IS_MENU_MANAGER (self), NULL);
...
/**
 * foundry_menu_manager_add_resource:
 * @self: a #FoundryMenuManager
 *
 ...
 */
guint
foundry_menu_manager_add_resource (FoundryMenuManager  *self,
...
/**
 * foundry_menu_manager_get_menu_ids:
 * @self: a #FoundryMenuManager
 *
 ...
 */
const char * const *
foundry_menu_manager_get_menu_ids (FoundryMenuManager *self)
{
  g_return_val_if_fail (FOUNDRY_IS_MENU_MANAGER (self), NULL);
...
/**
 * foundry_menu_manager_set_attribute_string:
 * @self: a #FoundryMenuManager
 * @menu: the menu that was retreived with foundry_menu_manager_get_menu_by_id()
 ...
 */
void
foundry_menu_manager_set_attribute_string (FoundryMenuManager *self,
                                           GMenu              *menu,
                                           guint               position,
...
/**
 * foundry_menu_manager_find_item_by_id:
 * @self: a #FoundryMenuManager
 ...
 */
GMenu *
foundry_menu_manager_find_item_by_id (FoundryMenuManager *self,
                                      const char         *id,
                                      guint              *position)
{
  GHashTableIter iter;
