/* foundry-markup-view.c
 *
 * Copyright 2018-2025 Christian Hergert <christian@sourceandstack.com>
 *
 ...
 */
static char *
parse_markdown (const char *markdown,
                gssize      len)
{
  ...
  FOUNDRY_RETURN (g_string_free (g_steal_pointer (&result), FALSE));
}
#endif

struct _FoundryMarkupView
{
  GtkWidget      parent_instance;
  FoundryMarkup *markup;
};

G_DEFINE_FINAL_TYPE (FoundryMarkupView, foundry_markup_view, GTK_TYPE_WIDGET)

static void
foundry_markup_view_dispose (GObject *object)
{
  FoundryMarkupView *self = FOUNDRY_MARKUP_VIEW (object);
  ...
  G_OBJECT_CLASS (foundry_markup_view_parent_class)->dispose (object);
}

static void
foundry_markup_view_class_init (FoundryMarkupViewClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (klass);

  object_class->dispose = foundry_markup_view_dispose;

  gtk_widget_class_set_css_name (widget_class, "FoundryMarkupView");
  gtk_widget_class_set_layout_manager_type (widget_class, GTK_TYPE_BIN_LAYOUT);
}

static void
foundry_markup_view_init (FoundryMarkupView *self)
{
}

GtkWidget *
foundry_markup_view_new (FoundryMarkup *content)
{
  g_autofree char *markup = NULL;
  gsize markup_len;
  GtkWidget *child = NULL;
  FoundryMarkupView *self;
  FoundryMarkupKind kind;

  g_return_val_if_fail (content != NULL, NULL);

  self = g_object_new (FOUNDRY_TYPE_MARKUP_VIEW, NULL);
  self->markup = g_object_ref (content);
  kind = foundry_markup_get_kind (content);
  markup = foundry_markup_to_string (content, &markup_len);
  ...
  switch (kind)
    {
    default:
    case FOUNDRY_MARKUP_KIND_MARKDOWN:
#ifdef HAVE_CMARK
      {
    ...
    case FOUNDRY_MARKUP_KIND_PLAINTEXT:
    case FOUNDRY_MARKUP_KIND_PANGO:
      {
        ...
        child = g_object_new (GTK_TYPE_LABEL,
                              ...
                              "xalign", 0.0f,
                              "visible", TRUE,
                              "use-markup", kind == FOUNDRY_MARKUP_KIND_PANGO,
                              "label", markup_nul_terminated,
                              NULL);
        break;
      }
    case FOUNDRY_MARKUP_KIND_HTML:
#ifdef HAVE_WEBKIT
...
/**
 * foundry_markup_view_dup_markup:
 * @self: a [class@FoundryGtk.MarkupView]
 *
 * Returns: (transfer full): a [class@Foundry.Markup]
 */
FoundryMarkup *
foundry_markup_view_dup_markup (FoundryMarkupView *self)
{
  g_return_val_if_fail (FOUNDRY_IS_MARKUP_VIEW (self), NULL);

  return g_object_ref (self->markup);
}
