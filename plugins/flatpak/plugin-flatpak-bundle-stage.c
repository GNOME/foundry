...
struct _PluginFlatpakBundleStage
{
...
G_DEFINE_FINAL_TYPE (PluginFlatpakBundleStage, plugin_flatpak_bundle_stage, FOUNDRY_TYPE_BUILD_STAGE)
...
static DexFuture *
plugin_flatpak_bundle_stage_build_fiber (gpointer user_data)
{
  ...
  PluginFlatpakBundleStage *self;
  FoundryBuildProgress *progress;

  g_assert (pair != NULL);
  g_assert (PLUGIN_IS_FLATPAK_BUNDLE_STAGE (pair->first));
  g_assert (FOUNDRY_IS_BUILD_PROGRESS (pair->second));

  self = PLUGIN_FLATPAK_BUNDLE_STAGE (pair->first);
...
static DexFuture *
plugin_flatpak_bundle_stage_build (FoundryBuildStage    *build_stage,
                                   FoundryBuildProgress *progress)
{
  PluginFlatpakBundleStage *self = (PluginFlatpakBundleStage *)build_stage;

  g_assert (PLUGIN_IS_FLATPAK_BUNDLE_STAGE (self));
  g_assert (FOUNDRY_IS_BUILD_PROGRESS (progress));

  return dex_scheduler_spawn (NULL, 0,
                              plugin_flatpak_bundle_stage_build_fiber,
                              foundry_pair_new (build_stage, progress),
                              (GDestroyNotify) foundry_pair_free);
}

static DexFuture *
plugin_flatpak_bundle_stage_query (FoundryBuildStage *build_stage)
{
...
plugin_flatpak_bundle_stage_get_phase (FoundryBuildStage *build_stage)
{
  return FOUNDRY_BUILD_PIPELINE_PHASE_EXPORT;
}

static void
plugin_flatpak_bundle_stage_finalize (GObject *object)
{
  PluginFlatpakBundleStage *self = (PluginFlatpakBundleStage *)object;

  g_clear_pointer (&self->repo_dir, g_free);
  g_clear_pointer (&self->staging_dir, g_free);
  g_clear_pointer (&self->state_dir, g_free);

  G_OBJECT_CLASS (plugin_flatpak_bundle_stage_parent_class)->finalize (object);
}

static void
plugin_flatpak_bundle_stage_get_property (GObject    *object,
                                          guint       prop_id,
                                          GValue     *value,
                                          GParamSpec *pspec)
{
  PluginFlatpakBundleStage *self = PLUGIN_FLATPAK_BUNDLE_STAGE (object);
...
static void
plugin_flatpak_bundle_stage_set_property (GObject      *object,
                                          guint         prop_id,
                                          const GValue *value,
                                          GParamSpec   *pspec)
{
...
static void
plugin_flatpak_bundle_stage_class_init (PluginFlatpakBundleStageClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  FoundryBuildStageClass *build_stage_class = FOUNDRY_BUILD_STAGE_CLASS (klass);

  object_class->finalize = plugin_flatpak_bundle_stage_finalize;
  object_class->get_property = plugin_flatpak_bundle_stage_get_property;
  object_class->set_property = plugin_flatpak_bundle_stage_set_property;

  build_stage_class->build = plugin_flatpak_bundle_stage_build;
  build_stage_class->query = plugin_flatpak_bundle_stage_query;
  build_stage_class->get_phase = plugin_flatpak_bundle_stage_get_phase;
...
static void
plugin_flatpak_bundle_stage_init (PluginFlatpakBundleStage *self)
{
}

FoundryBuildStage *
plugin_flatpak_bundle_stage_new (FoundryContext *context,
                                 ...
                                 const char     *repo_dir)
{
  ...
  return g_object_new (PLUGIN_TYPE_FLATPAK_BUNDLE_STAGE,
                       "context", context,
