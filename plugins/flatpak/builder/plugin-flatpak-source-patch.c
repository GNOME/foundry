...
struct _PluginFlatpakSourcePatch
{
...
G_DEFINE_FINAL_TYPE (PluginFlatpakSourcePatch, plugin_flatpak_source_patch, PLUGIN_TYPE_FLATPAK_SOURCE)

static void
plugin_flatpak_source_patch_finalize (GObject *object)
{
  PluginFlatpakSourcePatch *self = (PluginFlatpakSourcePatch *)object;

  g_clear_pointer (&self->path, g_free);
  g_clear_pointer (&self->paths, g_strfreev);
  g_clear_pointer (&self->options, g_strfreev);

  G_OBJECT_CLASS (plugin_flatpak_source_patch_parent_class)->finalize (object);
}

static void
plugin_flatpak_source_patch_get_property (GObject    *object,
                                        guint       prop_id,
                                        GValue     *value,
                                        GParamSpec *pspec)
{
  PluginFlatpakSourcePatch *self = PLUGIN_FLATPAK_SOURCE_PATCH (object);
...
static void
plugin_flatpak_source_patch_set_property (GObject      *object,
                                        guint         prop_id,
                                        const GValue *value,
                                        GParamSpec   *pspec)
{
...
static void
plugin_flatpak_source_patch_class_init (PluginFlatpakSourcePatchClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  PluginFlatpakSourceClass *source_class = PLUGIN_FLATPAK_SOURCE_CLASS (klass);

  object_class->finalize = plugin_flatpak_source_patch_finalize;
  object_class->get_property = plugin_flatpak_source_patch_get_property;
  object_class->set_property = plugin_flatpak_source_patch_set_property;
...
static void
plugin_flatpak_source_patch_init (PluginFlatpakSourcePatch *self)
{
}
