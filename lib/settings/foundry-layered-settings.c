...
struct _FoundryLayeredSettings
{
...
G_DEFINE_FINAL_TYPE (FoundryLayeredSettings, foundry_layered_settings, G_TYPE_OBJECT)
...
static GSettings *
foundry_layered_settings_get_primary_settings (FoundryLayeredSettings *self)
{
  g_assert (FOUNDRY_IS_LAYERED_SETTINGS (self));
...
static void
foundry_layered_settings_cache_key (FoundryLayeredSettings *self,
                                    const char             *key)
{
...
static void
foundry_layered_settings_update_cache (FoundryLayeredSettings *self)
{
  ...
  if ((keys = g_settings_schema_list_keys (schema)))
    {
      for (guint i = 0; keys[i]; i++)
        foundry_layered_settings_cache_key (self, keys [i]);
    }
}

static void
foundry_layered_settings_settings_changed_cb (FoundryLayeredSettings *self,
                                              const char             *key,
                                              GSettings              *settings)
{
  g_assert (FOUNDRY_IS_LAYERED_SETTINGS (self));
  ...
  foundry_layered_settings_cache_key (self, key);
}

static void
foundry_layered_settings_constructed (GObject *object)
{
  FoundryLayeredSettings *self = (FoundryLayeredSettings *)object;
  ...
  G_OBJECT_CLASS (foundry_layered_settings_parent_class)->constructed (object);
}

static void
foundry_layered_settings_finalize (GObject *object)
{
  FoundryLayeredSettings *self = (FoundryLayeredSettings *)object;
  ...
  G_OBJECT_CLASS (foundry_layered_settings_parent_class)->finalize (object);
}

static void
foundry_layered_settings_get_property (GObject    *object,
                                   guint       prop_id,
                                   GValue     *value,
                                   GParamSpec *pspec)
{
  FoundryLayeredSettings *self = FOUNDRY_LAYERED_SETTINGS (object);
...
static void
foundry_layered_settings_set_property (GObject      *object,
                                   guint         prop_id,
                                   const GValue *value,
                                   GParamSpec   *pspec)
{
...
static void
foundry_layered_settings_class_init (FoundryLayeredSettingsClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->constructed = foundry_layered_settings_constructed;
  object_class->finalize = foundry_layered_settings_finalize;
  object_class->get_property = foundry_layered_settings_get_property;
  object_class->set_property = foundry_layered_settings_set_property;
...
static void
foundry_layered_settings_init (FoundryLayeredSettings *self)
{
...
FoundryLayeredSettings *
foundry_layered_settings_new (const char *schema_id,
                              const char *path)
{
  g_return_val_if_fail (schema_id != NULL, NULL);
  g_return_val_if_fail (path != NULL, NULL);

  return g_object_new (FOUNDRY_TYPE_LAYERED_SETTINGS,
                       "schema-id", schema_id,
                       "path", path,
                       NULL);
}
...
GVariant *
foundry_layered_settings_get_default_value (FoundryLayeredSettings *self,
                                            const char             *key)
{
  g_return_val_if_fail (FOUNDRY_IS_LAYERED_SETTINGS (self), NULL);
  g_return_val_if_fail (key != NULL, NULL);

  return g_settings_get_default_value (foundry_layered_settings_get_primary_settings (self), key);
}

GVariant *
foundry_layered_settings_get_user_value (FoundryLayeredSettings *self,
                                         const char             *key)
{
  g_return_val_if_fail (FOUNDRY_IS_LAYERED_SETTINGS (self), NULL);
...
/**
 * foundry_layered_settings_get_value:
 * @self: a #FoundryLayeredSettings
 *
 ...
 */
GVariant *
foundry_layered_settings_get_value (FoundryLayeredSettings *self,
                                    const char             *key)
{
  g_return_val_if_fail (FOUNDRY_IS_LAYERED_SETTINGS (self), NULL);
  g_return_val_if_fail (key != NULL, NULL);
  ...
  return g_settings_get_value (foundry_layered_settings_get_primary_settings (self), key);
}

void
foundry_layered_settings_set_value (FoundryLayeredSettings *self,
                                    const char             *key,
                                    GVariant               *value)
{
  g_return_if_fail (FOUNDRY_IS_LAYERED_SETTINGS (self));
  g_return_if_fail (key != NULL);

  g_settings_set_value (foundry_layered_settings_get_primary_settings (self), key, value);
...
ret_type                                                                  \
foundry_layered_settings_get_##name (FoundryLayeredSettings *self,        \
                                 const char         *key)                 \
{                                                                         \
  GVariant *value;                                                        \
  ret_type ret;                                                           \
                                                                          \
  g_return_val_if_fail (FOUNDRY_IS_LAYERED_SETTINGS (self), (ret_type)0); \
...
{                                                                  \
  ...
  foundry_layered_settings_set_value (self, key, value);           \
}
...
void
foundry_layered_settings_append (FoundryLayeredSettings *self,
                                 GSettings              *settings)
{
  g_auto(GStrv) keys = NULL;
  ...
  keys = foundry_layered_settings_list_keys (self);
  for (guint i = 0; keys[i]; i++)
    {
  ...
  g_signal_connect_object (settings,
                           "changed",
                           G_CALLBACK (foundry_layered_settings_settings_changed_cb),
                           self,
                           G_CONNECT_SWAPPED);

  foundry_layered_settings_update_cache (self);
}
...
void
foundry_layered_settings_bind (FoundryLayeredSettings *self,
                               const char             *key,
                               gpointer                object,
                               const char             *property,
                               GSettingsBindFlags      flags)
{
  ...
  foundry_layered_settings_bind_with_mapping (self, key, object, property, flags,
                                              get_mapping, set_mapping, NULL, NULL);
}

/**
 * foundry_layered_settings_bind_with_mapping:
 * @self: An #FoundryLayeredSettings.
 ...
 */
void
foundry_layered_settings_bind_with_mapping (FoundryLayeredSettings  *self,
                                            const char              *key,
                                            gpointer                 object,
                                            const char              *property,
                                            GSettingsBindFlags       flags,
                                            ...
                                            GDestroyNotify           destroy)
{
  ...
  if ((flags & G_SETTINGS_BIND_SET) != 0)
    g_settings_bind_with_mapping (foundry_layered_settings_get_primary_settings (self),
                                  key, object, property, (flags & ~G_SETTINGS_BIND_GET),
                                  NULL, set_mapping, user_data, set_destroy);
}

void
foundry_layered_settings_unbind (FoundryLayeredSettings *self,
                                 const char             *property)
{
  g_return_if_fail (FOUNDRY_IS_LAYERED_SETTINGS (self));
  g_return_if_fail (property != NULL);

  g_settings_unbind (foundry_layered_settings_get_primary_settings (self), property);
  g_settings_unbind (memory_backend, property);
}

/**
 * foundry_layered_settings_get_key:
 * @self: a #FoundryLayeredSettings
 ...
 */
GSettingsSchemaKey *
foundry_layered_settings_get_key (FoundryLayeredSettings *self,
                                  const char             *key)
{
  g_autoptr(GSettingsSchema) schema = NULL;
...
/**
 * foundry_layered_settings_list_keys:
 * @self: a #FoundryLayeredSettings
 *
 * Lists the available keys.
 *
 * Returns: (transfer full) (array zero-terminated=1) (element-type utf8):
 *   an array of keys that can be retrieved from the #FoundryLayeredSettings.
 */
char **
foundry_layered_settings_list_keys (FoundryLayeredSettings *self)
{
  g_autoptr(GSettingsSchema) schema = NULL;
