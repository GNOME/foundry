...
foundry_process_launcher_end (FoundryProcessLauncher   *self,
                              char                   ***argv,
                              GError                  **error)
{
...
#ifdef FOUNDRY_ENABLE_TRACE
  {
    ...
    for (const GList *iter = self->layers.head;
         ...
         iter = iter->next)
      {
        FoundryProcessLauncherLayer *layer = iter->data;

        FOUNDRY_TRACE_MSG ("[%d]:    CWD: %s", j++, layer->cwd);
        FOUNDRY_TRACE_MSG ("        N FDS: %u", foundry_unix_fd_map_get_length (layer->unix_fd_map));
        FOUNDRY_TRACE_MSG ("  Environment:");
        for (guint i = 0; i < layer->env->len; i++)
          FOUNDRY_TRACE_MSG ("  [%02u]: %s", i, g_array_index (layer->env, char *, i));
        FOUNDRY_TRACE_MSG ("  Arguments:");
        for (guint i = 0; i < layer->argv->len; i++)
          FOUNDRY_TRACE_MSG ("  [%02u]: %s ", i, g_array_index (layer->argv, char *, i));
      }
  }
#endif
  ...
  *argv = g_strdupv ((char **)foundry_process_launcher_get_argv (self));
