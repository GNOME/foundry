...
static int
foundry_cli_builtin_vcs_list_remotes_run (FoundryCommandLine *command_line,
                                          ...
                                          DexCancellable     *cancellable)
{
  ...
  if (!(list = dex_await_object (foundry_vcs_list_remotes (vcs), &error)))
    goto handle_error;

  format_arg = foundry_cli_options_get_string (options, "format");
  format = foundry_object_serializer_format_parse (format_arg);

  foundry_command_line_print_list (command_line, list, fields, format, FOUNDRY_TYPE_VCS_REMOTE);
...
void
foundry_cli_builtin_vcs_list_remotes (FoundryCliCommandTree *tree)
{
  foundry_cli_command_tree_register (tree,
                                     FOUNDRY_STRV_INIT ("foundry", "vcs", "list-remotes"),
                                     &(FoundryCliCommand) {
                                       ...
                                       .run = foundry_cli_builtin_vcs_list_remotes_run,
                                       .prepare = NULL,
                                       .complete = NULL,
                                       .gettext_package = GETTEXT_PACKAGE,
                                       .description = N_("List remotes known to the VCS"),
                                     });
}
