                                  ...
                                  DexCancellable     *cancellable)
{
  ...
  if (!(list = dex_await_object (foundry_llm_manager_list_models (llm_manager), &error)))
    goto handle_error;

  format_arg = foundry_cli_options_get_string (options, "format");
  format = foundry_object_serializer_format_parse (format_arg);
  foundry_command_line_print_list (command_line, list, fields, format, FOUNDRY_TYPE_LLM_MODEL);
...
foundry_cli_builtin_llm_list (FoundryCliCommandTree *tree)
{
  foundry_cli_command_tree_register (tree,
                                     ...
                                     &(FoundryCliCommand) {
                                       ...
                                       .description = N_("List available language models"),
                                     });
}
