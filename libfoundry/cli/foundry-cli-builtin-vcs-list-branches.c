...
static int
foundry_cli_builtin_vcs_list_branches_run (FoundryCommandLine *command_line,
                                           ...
                                           DexCancellable     *cancellable)
{
  ...
  if (!(list = dex_await_object (foundry_vcs_list_branches (vcs), &error)))
    goto handle_error;

  format_arg = foundry_cli_options_get_string (options, "format");
  format = foundry_object_serializer_format_parse (format_arg);

  foundry_command_line_print_list (command_line, list, fields, format, FOUNDRY_TYPE_VCS_BRANCH);
...
void
foundry_cli_builtin_vcs_list_branches (FoundryCliCommandTree *tree)
{
  foundry_cli_command_tree_register (tree,
                                     FOUNDRY_STRV_INIT ("foundry", "vcs", "list-branches"),
                                     &(FoundryCliCommand) {
                                       ...
                                       .run = foundry_cli_builtin_vcs_list_branches_run,
                                       .prepare = NULL,
                                       .complete = NULL,
