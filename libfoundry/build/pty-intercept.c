...
static void     _pty_intercept_sclose (PtyInterceptSide *side);
 ...
 */
int
pty_intercept_create_producer (int consumer_fd,
                                   gboolean blocking)
{
  g_autofd int ret = PTY_FD_INVALID;
 ...
 */
int
pty_intercept_create_consumer (void)
{
  g_autofd int consumer_fd = PTY_FD_INVALID;
   ...
   */
  if (consumer_fd == PTY_FD_INVALID && errno == EINVAL)
    {
...
static void
_pty_intercept_sclose (PtyInterceptSide *side)
{
  g_assert (side != NULL);
...
_pty_intercept_out_cb (GIOChannel   *channel,
                       ...
                       gpointer      user_data)
{
  PtyIntercept *self = user_data;
  PtyInterceptSide *us, *them;
  GIOStatus status;
  const gchar *wrbuf;
  gsize n_written = 0;
  gsize len = 0;
...
close_and_cleanup:
  ...
  _pty_intercept_sclose (us);
  _pty_intercept_sclose (them);
                      ...
                      gpointer      user_data)
{
  ...
  g_assert (IS_PTY_INTERCEPT (self));
 ...
 */
gboolean
pty_intercept_set_size (PtyIntercept *self,
                        guint         rows,
                        guint         columns)
{

  g_return_val_if_fail (IS_PTY_INTERCEPT (self), FALSE);

  if (self->consumer.channel != NULL)
    {
      int fd = g_io_channel_unix_get_fd (self->consumer.channel);
      struct winsize ws = {0};
 ...
 */
gboolean
pty_intercept_init (PtyIntercept *self,
                    int           fd,
                    GMainContext *main_context)
{
  g_autofd int producer_fd = PTY_FD_INVALID;
  g_autofd int consumer_fd = PTY_FD_INVALID;
  struct winsize ws;
  ...
  producer_fd = pty_intercept_create_producer (fd, FALSE);
  if (producer_fd == PTY_FD_INVALID)
    return FALSE;
   ...
   */
  if (!_pty_intercept_set_raw (producer_fd))
    return FALSE;

  consumer_fd = pty_intercept_create_consumer ();
  if (consumer_fd == PTY_FD_INVALID)
    return FALSE;
  ...
  self->consumer.channel = g_io_channel_unix_new (g_steal_fd (&consumer_fd));
  self->producer.channel = g_io_channel_unix_new (g_steal_fd (&producer_fd));
 ...
 * @self: a #PtyIntercept
 *
 * Cleans up a #PtyIntercept previously initialized with
 * pty_intercept_init().
 *
 ...
 */
void
pty_intercept_clear (PtyIntercept *self)
{
  g_return_if_fail (IS_PTY_INTERCEPT (self));
 ...
 * @self: a #PtyIntercept
 *
 * Gets a consumer PTY fd created by the #PtyIntercept. This is suitable
 ...
 */
int
pty_intercept_get_fd (PtyIntercept *self)
{
  g_return_val_if_fail (IS_PTY_INTERCEPT (self), PTY_FD_INVALID);
  g_return_val_if_fail (self->consumer.channel != NULL, PTY_FD_INVALID);
 ...
 */
void
pty_intercept_set_callback (PtyIntercept         *self,
