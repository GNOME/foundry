...
Once a session is open, send prompts as [struct@Foundry.AcpContentBlock]
arrays. The returned future resolves to [struct@Foundry.AcpPromptResult].
...
[struct@Foundry.AcpPermissionRequest]. The application chooses an option and
returns [struct@Foundry.AcpPermissionResponse].
...
Updates are represented by [struct@Foundry.AcpSessionUpdate] and include message
